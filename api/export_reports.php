...
foreach ($records as $record) {
    ...
    secure_fputcsv($output, $row);
}

// Add summary statistics at the bottom
secure_fputcsv($output, []); // Empty row
secure_fputcsv($output, ['SUMMARY STATISTICS']);
secure_fputcsv($output, ['Total Records', count($records)]);
...
secure_fputcsv($output, ['Completed Forms', $completed]);
secure_fputcsv($output, ['Draft Forms', $draft]);
secure_fputcsv($output, ['Archived Forms', $archived]);

secure_fputcsv($output, []); // Empty row
secure_fputcsv($output, ['Generated on', date('Y-m-d H:i:s')]);
secure_fputcsv($output, ['Generated by', $current_user['full_name'] . ' (' . $current_user['username'] . ')']);
secure_fputcsv($output, ['Date Range', $date_from . ' to ' . $date_to]);
