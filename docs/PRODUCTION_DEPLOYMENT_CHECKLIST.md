...
- **Patient data in browser storage:** `public/js/modules/auto-save.js` currently stores unsaved form fields and injury data in `localStorage` under `prehospital_autosave_draft`. This can leave patient-identifying or medical information on the workstation after the browser is closed. Prefer protected server-side drafts, or document an approved risk decision for tightly controlled offline workstations. Clear any existing browser storage when migrating.
